...
function generateDXF2007($entities, $angleUnit = 'deg', $printScale = 100, $paperSizeKey = 'A3-L', $paperFrameCenterX = null, $paperFrameCenterY = null) {
    ...
    foreach ($titleBoardDefs as $boardDef) {
        ...
        $dxf[] = "0{$nl}BLOCK"; $dxf[] = "5{$nl}{$boardDef['blockHandle']}"; $dxf[] = "330{$nl}{$record}";
        $dxf[] = "100{$nl}AcDbEntity"; $dxf[] = "8{$nl}0"; $dxf[] = "100{$nl}AcDbBlockBegin";
        $dxf[] = "2{$nl}{$boardDef['name']}"; $dxf[] = "70{$nl}0"; $dxf[] = "10{$nl}0.0000"; $dxf[] = "20{$nl}0.0000"; $dxf[] = "30{$nl}0.0000";
        $dxf[] = "3{$nl}{$boardDef['name']}"; $dxf[] = "1{$nl}";
        foreach ($boardDef['children'] as $child) {
            ...
            if ($type === 'line') {
                ...
                $dxf[] = "100{$nl}AcDbEntity"; $dxf[] = "8{$nl}0"; $dxf[] = "62{$nl}{$color}"; $dxf[] = "100{$nl}AcDbLine";
                $dxf[] = "10{$nl}" . sprintf('%.4f', (float)$child['x1'] - $center['x']); $dxf[] = "20{$nl}" . sprintf('%.4f', (float)$child['y1'] - $center['y']);
                $dxf[] = "11{$nl}" . sprintf('%.4f', (float)$child['x2'] - $center['x']); $dxf[] = "21{$nl}" . sprintf('%.4f', (float)$child['y2'] - $center['y']);
                ...
                $dxf[] = "100{$nl}AcDbEntity"; $dxf[] = "8{$nl}0"; $dxf[] = "62{$nl}{$color}"; $dxf[] = "100{$nl}AcDbPolyline";
                $dxf[] = "90{$nl}" . count($points); $dxf[] = "70{$nl}1";
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; }
        ...
        .toolbar-collapse { border-right: 1px solid var(--border-color); padding: 4px 8px 4px 0; }
        .toolbar-collapse > summary { cursor: pointer; color: var(--text-muted); font-size: 10px; font-weight: 600; text-transform: uppercase; list-style: none; padding: 3px 0; }
        .toolbar-collapse > summary::-webkit-details-marker { display: none; }
        .toolbar-collapse > summary::before { content: '▸ '; color: var(--accent); }
        .toolbar-collapse[open] > summary::before { content: '▾ '; }
        .toolbar-collapse[open] > .btn-group { border: 0; padding: 2px 0 0; }
        ...
        .panel-tabs { display: flex; align-items: stretch; }
        .panel-tab {
        ...
        .panel-tab:hover { background: #363639; color: var(--text-main); }
        .panel-tab.active {
            background: var(--bg-panel);
        ...
        .help-tool summary::-webkit-details-marker { display: none; }
        .help-tool summary::after {
            content: '+';
            position: absolute;
            right: 8px;
            color: var(--text-muted);
        ...
        .help-tool[open] summary {
            background: #2d2d30;
        }
        .help-tool[open] summary::after { content: '−'; }
        .help-tool-content {
            min-height: 12px;
            padding: 8px;
            color: var(--text-muted);
        ...
        #help-container { padding: 4px; }
        .help-tool-content h4 {
            margin: 10px 0 4px;
            color: var(--text-main);
            font-size: 11px;
        }
        .help-tool-content h4:first-child { margin-top: 0; }
        .help-tool-content p { margin: 0 0 8px; }
        .help-tool-content ul {
            margin: 4px 0 10px;
            padding-left: 18px;
        }
        .help-tool-content li { margin-bottom: 4px; }
        .help-tool-content kbd {
            padding: 1px 4px;
            border: 1px solid #555;
...
<div id="toolbar">
    <div class="btn-group">
        ...
        <button id="tool-arc" class="tool-btn icon-btn" data-tool="arc" title="Arc (3 Point)"><svg viewBox="0 0 24 24"><path d="M4 17a9 9 0 0 1 13-10"/></svg><span class="sr-only">Arc</span></button>
    ...
    <div style="display:none" aria-hidden="true">
        ...
        <select id="paperSize">
            ...
            <option value="A2-P">A2 P</option><option value="A2-L">A2 L</option>
            <option value="A1-P">A1 P</option><option value="A1-L">A1 L</option>
            <option value="A0-P">A0 P</option><option value="A0-L">A0 L</option>
        </select>
        <select id="printScale">
            <option value="50">Scale 1:50</option><option value="100" selected>Scale 1:100</option>
        ...
        <input type="checkbox" id="osnapToggle" checked>
...
<div id="main-container">
    ...
    <div id="properties-palette">
        <div class="panel-header">
            <div class="panel-tabs" role="tablist" aria-label="Properties panel">
                <button id="properties-tab" class="panel-tab active" type="button" role="tab" aria-selected="true" aria-controls="properties-container">PROPERTIES</button>
                <button id="help-tab" class="panel-tab" type="button" role="tab" aria-selected="false" aria-controls="help-container">HELP</button>
            </div>
            <span class="panel-header-actions"><span id="prop-entity-count" style="color: var(--text-muted);">No selection</span> <button id="toggle-properties" title="Collapse properties">−</button></span>
        </div>
        <div class="panel-content" id="properties-container" role="tabpanel" aria-labelledby="properties-tab">
            <div style="color: var(--text-muted); text-align: center; margin-top: 40px;">
                Select an entity to view and edit its properties.
            </div>
        </div>
        <div class="panel-content" id="help-container" role="tabpanel" aria-labelledby="help-tab" hidden>
            <div id="help-tools"></div>
        </div>
    </div>
</div>
...
<div id="statusbar">
    ...
    <div>
        ...
        <button type="button" id="status-osnap" class="osnap-badge status-toggle" aria-pressed="true" title="Toggle OSNAP (F3)">OSNAP: ON</button>
        <button type="button" id="status-snap-grid" class="osnap-badge status-toggle" aria-pressed="false" title="Toggle Grid Snap">SNAP: OFF</button>
        <button type="button" id="status-ortho" class="osnap-badge status-toggle" aria-pressed="false" title="Toggle Ortho (F8)">ORTHO: OFF</button>
...
<div id="point-import-modal" role="dialog" aria-modal="true" aria-labelledby="point-import-title">
    <div class="point-import-panel">
        ...
        <div id="point-import-fields" hidden>
            ...
            <label><input type="checkbox" id="point-import-no-contours"> No Contours</label>
        </div>
...
(() => {
    ...
    const osnapToggle = document.getElementById('osnapToggle');
    const snapGridToggle = document.getElementById('snapGrid');
    const orthoToggle = document.getElementById('orthoToggle');
    const statusOsnap = document.getElementById('status-osnap');
    const statusSnapGrid = document.getElementById('status-snap-grid');
    ...
    const pointTypes = [
        ...
        ['block-vertex', 'Κορφή Ο.Τ.'],
        ['building-vertex', 'Κορφή οικ.']
    ];
    ...
    const helpContentByTool = {
        'tool-select': `
            <h4>Σκοπός και ενεργοποίηση</h4>
            <p>Το Select είναι το βασικό εργαλείο επιλογής και άμεσης επεξεργασίας αντικειμένων. Ενεργοποιείται από το εικονίδιο του δείκτη και γίνεται ξανά ενεργό αυτόματα μετά την ολοκλήρωση πολλών εντολών δημιουργίας ή τροποποίησης.</p>

            <h4>Απλή επιλογή</h4>
            <ul>
                <li><strong>Αριστερό κλικ σε αντικείμενο:</strong> αντικαθιστά την προηγούμενη επιλογή με το αντικείμενο που εντοπίστηκε.</li>
                <li><strong>Αριστερό κλικ σε κενό χώρο:</strong> καθαρίζει την επιλογή και ξεκινά πιθανό παράθυρο επιλογής.</li>
                <li>Το επιλεγμένο αντικείμενο επισημαίνεται στο σχέδιο και οι ιδιότητές του εμφανίζονται στην καρτέλα <strong>PROPERTIES</strong>.</li>
                <li>Σε πολιγραμμή επιλέγεται και το τμήμα πάνω στο οποίο έγινε το κλικ. Το ενεργό τμήμα επισημαίνεται και εμφανίζονται οι συντεταγμένες άκρων, ΔX, ΔY, μήκος και αζιμούθιό του.</li>
                <li>Το hatch μπορεί να επιλεγεί ανεξάρτητα από το αντικείμενο που το φέρει, ώστε να εμφανιστούν οι δικές του ιδιότητες.</li>
            </ul>

            <h4>Πολλαπλή επιλογή</h4>
            <ul>
                <li><kbd>Shift</kbd> + κλικ προσθέτει ένα αντικείμενο στην επιλογή.</li>
                <li><kbd>Shift</kbd> + κλικ σε ήδη επιλεγμένο αντικείμενο το αφαιρεί από την επιλογή.</li>
                <li><kbd>Ctrl</kbd> + <kbd>A</kbd> επιλέγει όλα τα αντικείμενα του σχεδίου.</li>
                <li>Στην πολλαπλή επιλογή εμφανίζονται οι γενικές ιδιότητες Color, Line Weight, Layer και Locked, καθώς και μόνο οι υπόλοιπες ιδιότητες που είναι κοινές σε όλα τα επιλεγμένα αντικείμενα.</li>
                <li>Όταν οι επιλεγμένες οντότητες έχουν διαφορετική τιμή, το πεδίο εμφανίζεται ως <strong>Mixed</strong>. Η εισαγωγή νέας τιμής την εφαρμόζει σε όλες.</li>
            </ul>

            <h4>Παράθυρο επιλογής</h4>
            <ul>
                <li>Πάτημα σε κενό χώρο και σύρσιμο δημιουργεί ορθογώνιο παράθυρο επιλογής.</li>
                <li>Επιλέγονται μόνο τα αντικείμενα των οποίων ολόκληρο το γεωμετρικό περίγραμμα βρίσκεται μέσα στο παράθυρο.</li>
                <li>Η κατεύθυνση του συρσίματος δεν αλλάζει τη λειτουργία: πρόκειται πάντα για πλήρη επιλογή παραθύρου και όχι για crossing selection.</li>
                <li>Με πατημένο <kbd>Shift</kbd>, τα αντικείμενα του παραθύρου προστίθενται στην υπάρχουσα επιλογή. Χωρίς <kbd>Shift</kbd>, την αντικαθιστούν.</li>
            </ul>

            <h4>Άμεση μετακίνηση με drag</h4>
            <ul>
                <li>Πάτημα πάνω σε επιλεγμένο, ξεκλείδωτο αντικείμενο και σύρσιμο μετακινεί άμεσα την επιλογή.</li>
                <li>Στην πολλαπλή επιλογή μετακινούνται μαζί όλα τα ξεκλείδωτα αντικείμενα, διατηρώντας τις σχετικές θέσεις τους.</li>
                <li>Η μετακίνηση ακολουθεί τα ενεργά OSNAP σημεία και καταγράφεται στο Undo μόλις ξεπεραστεί το μικρό όριο έναρξης του drag.</li>
                <li>Αν ένα αντικείμενο έχει <strong>Locked</strong>, δεν μετακινείται με άμεσο drag και δεν εμφανίζει ενεργά grips. Παραμένει όμως επιλέξιμο και οι ιδιότητές του μπορούν να αλλάξουν χειροκίνητα.</li>
                <li>Το Locked αφορά το drag. Η ξεχωριστή εντολή <strong>Move</strong> παραμένει διαθέσιμη για ελεγχόμενη μετακίνηση.</li>
            </ul>

            <h4>Grips και άμεση αλλαγή γεωμετρίας</h4>
            <p>Τα grips εμφανίζονται όταν είναι επιλεγμένο ένα μόνο ξεκλείδωτο αντικείμενο. Σύρονται με το ποντίκι, χρησιμοποιούν OSNAP και, όπου εφαρμόζεται, ORTHO.</p>
            <ul>
                <li><strong>Line:</strong> πράσινο Start και πορτοκαλί End αλλάζουν τα άκρα· το μπλε Mid μετακινεί ολόκληρη τη γραμμή.</li>
                <li><strong>Rectangle:</strong> τα τέσσερα γωνιακά grips αλλάζουν τις διαστάσεις με αγκυρωμένη την απέναντι γωνία. Τα τέσσερα ενδιάμεσα grips μετακινούν μία ολόκληρη πλευρά παράλληλα προς την απέναντι, διατηρώντας το μήκος της. Το κεντρικό grip μετακινεί ολόκληρο το ορθογώνιο.</li>
                <li><strong>Polyline:</strong> το κεντρικό grip μετακινεί όλη την πολιγραμμή, κάθε vertex grip μετακινεί μία κορυφή και κάθε edge grip μετακινεί μαζί τα δύο άκρα του αντίστοιχου τμήματος.</li>
                <li><strong>Circle:</strong> το κέντρο μετακινεί τον κύκλο· τα τέσσερα περιφερειακά grips αλλάζουν την ακτίνα.</li>
                <li><strong>Ellipse:</strong> το κέντρο μετακινεί την έλλειψη· τα οριζόντια grips αλλάζουν Rx και τα κατακόρυφα Ry.</li>
                <li><strong>Arc:</strong> Start και End αλλάζουν τα άκρα, το Center κινείται στη μεσοκάθετο κρατώντας τα άκρα αγκυρωμένα και το Mid επανυπολογίζει ολόκληρο το τόξο από Start–Mid–End με σταθερά άκρα.</li>
                <li><strong>Point:</strong> το κεντρικό grip μετακινεί το σημείο.</li>
                <li><strong>Text:</strong> το anchor μετακινεί το κείμενο. Στο multiline text, τα γωνιακά και ενδιάμεσα grips μεταβάλλουν επίσης το πλαίσιο κειμένου.</li>
                <li><strong>Distance Dimension:</strong> τα P1/P2 αλλάζουν τα σημεία μέτρησης και το position grip μετακινεί το κείμενο της διάστασης.</li>
                <li><strong>Angle Dimension:</strong> το Center μετακινεί την κορυφή και επανυπολογίζει τις ακτίνες, τα Ray 1/Ray 2 αλλάζουν τα σκέλη και το position grip μετακινεί την ένδειξη.</li>
                <li><strong>Imported drawing / title board:</strong> εμφανίζονται grips στα άκρα και στα μέσα του περιγράμματος του σύνθετου αντικειμένου.</li>
            </ul>

            <h4>Ιδιότητες της επιλογής</h4>
            <ul>
                <li><strong>Color:</strong> χρώμα εμφάνισης του αντικειμένου.</li>
                <li><strong>Line Weight:</strong> πάχος γραμμής.</li>
                <li><strong>Layer:</strong> ανάθεση σε ένα από τα layers του σχεδίου.</li>
                <li><strong>Locked:</strong> προστασία από τυχαίο drag και επεξεργασία με grips.</li>
                <li>Για μία μόνο επιλογή εμφανίζονται επιπλέον όλες οι ειδικές γεωμετρικές και λειτουργικές ιδιότητες του συγκεκριμένου τύπου αντικειμένου.</li>
                <li>Όλες οι χειροκίνητες αλλαγές ιδιοτήτων παραμένουν διαθέσιμες και σε Locked αντικείμενα.</li>
            </ul>

            <h4>Query Selection</h4>
            <p>Η ενότητα Query Selection της καρτέλας PROPERTIES επιλέγει αντικείμενα βάσει κριτηρίων Object, Property, Operator και Value. Πολλαπλά κριτήρια συνδυάζονται με AND. Το Select αντικαθιστά την τρέχουσα επιλογή με όλα τα αντικείμενα που ικανοποιούν τα κριτήρια.</p>

            <h4>Πλήκτρα και σχετικές ενέργειες</h4>
            <ul>
                <li><kbd>Esc</kbd>: ακυρώνει ενεργό grip ή εντολή και καθαρίζει την επιλογή. Κατά την ακύρωση grip επαναφέρει την αρχική γεωμετρία.</li>
            ...
            <p>Με το Select μπορεί να συρθεί το περίγραμμα του print frame, όταν το κλικ δεν συμπίπτει με αντικείμενο, ώστε να αλλάξει η θέση του χαρτιού.</p>
            <div class="help-note">Οι αλλαγές από drag, grips, ιδιότητες και διαγραφή συμμετέχουν στο Undo/Redo και στην αυτόματη αποθήκευση του σχεδίου.</div>
        `,
        'tool-line': `
            <h4>Σχεδίαση γραμμής</h4>
            <ol>
                <li>Επίλεξε το εργαλείο <strong>Line</strong> από την μπάρα εργαλείων.</li>
                <li>Κάνε κλικ για να ορίσεις το πρώτο άκρο <strong>P1 (Start)</strong>.</li>
                <li>Μετακίνησε τον δείκτη για να δεις ζωντανή προεπισκόπηση της γραμμής, του μήκους και του αζιμουθίου.</li>
                <li>Κάνε δεύτερο κλικ για να ορίσεις το <strong>P2 (End)</strong> και να ολοκληρώσεις τη γραμμή.</li>
            </ol>
            <p>Μετά το δεύτερο κλικ η γραμμή δημιουργείται, επιλέγεται αυτόματα και η εφαρμογή επιστρέφει σε λειτουργία Select.</p>

            <h4>Snapping και περιορισμοί σχεδίασης</h4>
            <ul>
                <li>Και τα δύο άκρα μπορούν να τοποθετηθούν πάνω στα ενεργά σημεία OSNAP άλλων αντικειμένων.</li>
                <li>Με ενεργό <strong>Grid Snap</strong>, οι συντεταγμένες ακολουθούν το πλέγμα όταν δεν υπερισχύει κάποιο OSNAP.</li>
                <li>Με ενεργό <strong>ORTHO</strong> ή <kbd>F8</kbd>, το δεύτερο άκρο περιορίζεται οριζόντια ή κατακόρυφα ως προς το P1.</li>
                <li>Κατά την προεπισκόπηση, η γραμμή ακολουθεί την ίδια τελική λογική ORTHO που θα εφαρμοστεί στο δεύτερο κλικ.</li>
                <li>Η γραμμή παρέχει ως δικά της OSNAP σημεία τα δύο endpoints, το midpoint και τα σημεία τομής της με άλλα υποστηριζόμενα αντικείμενα.</li>
            </ul>

            <h4>Αρχικές ιδιότητες</h4>
            <ul>
                <li><strong>Color:</strong> λαμβάνεται από το ενεργό Color του Paperspace.</li>
                <li><strong>Line Weight:</strong> λαμβάνεται από το ενεργό πάχος γραμμής του Paperspace.</li>
                <li><strong>Layer:</strong> κάθε νέα γραμμή ανήκει στο ενεργό/default layer του σχεδίου· μπορεί να μεταφερθεί σε οποιοδήποτε ορισμένο layer από τις Properties.</li>
                <li><strong>Locked:</strong> είναι αρχικά απενεργοποιημένο και μπορεί να ενεργοποιηθεί μετά τη δημιουργία.</li>
            </ul>

            <h4>General Properties</h4>
            <ul>
                <li><strong>Color:</strong> αλλάζει το χρώμα εμφάνισης της γραμμής.</li>
                <li><strong>Line Weight:</strong> αλλάζει το οπτικό πάχος της γραμμής.</li>
                <li><strong>Layer:</strong> αναθέτει τη γραμμή σε ένα από τα layers του τρέχοντος σχεδίου.</li>
                <li><strong>Locked:</strong> αποτρέπει την τυχαία μετακίνηση με drag και κρύβει/απενεργοποιεί τα grips. Δεν εμποδίζει τη χειροκίνητη αλλαγή ιδιοτήτων ή τη ρητή εντολή Move.</li>
            </ul>

            <h4>Line Geometry</h4>
            <ul>
                <li><strong>Start X / Start Y (P1):</strong> συντεταγμένες του πράσινου αρχικού άκρου. Είναι άμεσα επεξεργάσιμες.</li>
                <li><strong>End X / End Y (P2):</strong> συντεταγμένες του πορτοκαλί τελικού άκρου. Είναι άμεσα επεξεργάσιμες.</li>
                <li><strong>Delta X:</strong> υπολογιζόμενη οριζόντια διαφορά <code>X2 − X1</code>. Είναι μόνο για ανάγνωση.</li>
                <li><strong>Delta Y:</strong> υπολογιζόμενη κατακόρυφη διαφορά <code>Y2 − Y1</code>. Είναι μόνο για ανάγνωση.</li>
                <li><strong>Length (S):</strong> πραγματικό μήκος της γραμμής. Όταν αλλάξει, το P1 παραμένει αγκυρωμένο και το P2 μετακινείται πάνω στο υπάρχον αζιμούθιο. Η ελάχιστη τιμή είναι 0,001.</li>
                <li><strong>Azimuth:</strong> διεύθυνση από P1 προς P2. Όταν αλλάξει, το P1 και το μήκος παραμένουν σταθερά και επανυπολογίζεται το P2.</li>
                <li>Η μονάδα του Azimuth ακολουθεί την επιλογή <strong>Angle unit</strong> του Paperspace: μοίρες, βαθμοί gon ή ακτίνια.</li>
                <li><strong>Reverse Direction (P1 ↔ P2):</strong> ανταλλάσσει τα δύο άκρα. Η θέση και το μήκος δεν αλλάζουν, ενώ το αζιμούθιο αντιστρέφεται κατά μισό πλήρη κύκλο.</li>
            </ul>

            <h4>Grips</h4>
            <ul>
                <li><strong>Πράσινο P1 (Start):</strong> μετακινεί μόνο το αρχικό άκρο.</li>
                <li><strong>Πορτοκαλί P2 (End):</strong> μετακινεί μόνο το τελικό άκρο.</li>
                <li><strong>Μπλε Mid:</strong> μετακινεί ολόκληρη τη γραμμή χωρίς να αλλάζει μήκος, αζιμούθιο ή σχετική θέση των άκρων.</li>
                <li>Τα grips χρησιμοποιούν τα ενεργά OSNAP και ORTHO. Δεν είναι διαθέσιμα όταν η γραμμή είναι Locked ή όταν έχουν επιλεγεί πολλά αντικείμενα.</li>
            </ul>

            <h4>Επιλογή και πολλαπλή επεξεργασία</h4>
            <ul>
                <li>Η γραμμή επιλέγεται με κλικ κοντά σε οποιοδήποτε σημείο του τμήματός της.</li>
                <li>Μπορεί να προστεθεί ή να αφαιρεθεί από πολλαπλή επιλογή με <kbd>Shift</kbd> + κλικ.</li>
                <li>Με window selection επιλέγεται όταν και τα δύο άκρα—άρα ολόκληρη η γραμμή—βρίσκονται μέσα στο παράθυρο.</li>
                <li>Σε πολλαπλή επιλογή μπορούν να αλλάξουν τα General Properties και όλες οι primitive ιδιότητες που είναι κοινές σε όλα τα επιλεγμένα αντικείμενα.</li>
            </ul>

            <h4>Query Selection</h4>
            <p>Η γραμμή μπορεί να αναζητηθεί με κριτήρια για <strong>X1, Y1, X2, Y2, Delta X, Delta Y, Length, Azimuth, Color, Line Weight, Layer</strong> και <strong>Locked</strong>. Τα Delta X, Delta Y, Length και Azimuth υπολογίζονται δυναμικά από την τρέχουσα γεωμετρία. Πολλαπλά κριτήρια συνδυάζονται με AND.</p>
            ...
            <h4>Διαστάσεις και εξαγωγή</h4>
            <ul>
                <li>Τα endpoints και το midpoint της γραμμής μπορούν να χρησιμοποιηθούν για τη δημιουργία distance ή angle dimensions.</li>
                <li>Η γραμμή περιλαμβάνεται στην αποθήκευση JSON, στην εξαγωγή DXF, στην εκτύπωση PDF και στην αντιγραφή επιλεγμένης περιοχής ως εικόνα.</li>
            </ul>

            <h4>Ακύρωση, διαγραφή και ιστορικό</h4>
            <ul>
                <li><kbd>Esc</kbd> πριν από το δεύτερο κλικ ακυρώνει τη σχεδίαση.</li>
                <li><kbd>Delete</kbd> ή <kbd>Backspace</kbd> διαγράφει την επιλεγμένη γραμμή.</li>
                <li>Η δημιουργία και όλες οι γεωμετρικές αλλαγές συμμετέχουν στο Undo/Redo και στην αυτόματη αποθήκευση.</li>
            </ul>
            <div class="help-note">Το P1 ορίζει την αρχή της διεύθυνσης. Για αυτό οι αλλαγές Length και Azimuth κρατούν σταθερό το P1 και μετακινούν το P2.</div>
        `
    };

    function populateHelpTools() {
        helpTools.innerHTML = '';
        const savedHelpTool = localStorage.getItem('cad_help_open_tool');
        document.querySelectorAll('#toolbar button[title]').forEach(toolButton => {
            const section = document.createElement('details');
            section.className = 'help-tool';
            section.dataset.helpTool = toolButton.id;
            section.open = toolButton.id === savedHelpTool;

            const title = document.createElement('summary');
            title.textContent = toolButton.title;

            const content = document.createElement('div');
            content.className = 'help-tool-content';
            content.innerHTML = helpContentByTool[toolButton.id] || '';

            section.append(title, content);
            section.addEventListener('toggle', () => {
                if (section.open) {
                    helpTools.querySelectorAll('.help-tool[open]').forEach(otherSection => {
                        if (otherSection !== section) otherSection.open = false;
                    });
                    localStorage.setItem('cad_help_open_tool', toolButton.id);
                } else if (localStorage.getItem('cad_help_open_tool') === toolButton.id) {
                    localStorage.removeItem('cad_help_open_tool');
                }
            });
            helpTools.appendChild(section);
        });
    }

    function selectPropertiesPanelTab(tabName) {
        const showHelp = tabName === 'help';
        propertiesTab.classList.toggle('active', !showHelp);
        propertiesTab.setAttribute('aria-selected', showHelp ? 'false' : 'true');
        helpTab.classList.toggle('active', showHelp);
        helpTab.setAttribute('aria-selected', showHelp ? 'true' : 'false');
        propContainer.hidden = showHelp;
        helpContainer.hidden = !showHelp;
        propCount.style.display = showHelp ? 'none' : '';
        localStorage.setItem('cad_properties_panel_tab', showHelp ? 'help' : 'properties');
    }
    ...
    propertiesTab.addEventListener('click', () => selectPropertiesPanelTab('properties'));
    helpTab.addEventListener('click', () => selectPropertiesPanelTab('help'));
    populateHelpTools();
    selectPropertiesPanelTab(localStorage.getItem('cad_properties_panel_tab') === 'help' ? 'help' : 'properties');
    ...
    const savedOsnapEnabled = localStorage.getItem('cad_osnap_enabled');
    const savedSnapGridEnabled = localStorage.getItem('cad_snap_grid_enabled');
    const savedOrthoEnabled = localStorage.getItem('cad_ortho_enabled');
    if (savedOsnapEnabled !== null) osnapToggle.checked = savedOsnapEnabled === 'true';
    if (savedSnapGridEnabled !== null) snapGridToggle.checked = savedSnapGridEnabled === 'true';
    if (savedOrthoEnabled !== null) orthoToggle.checked = savedOrthoEnabled === 'true';
    ...
    function updateDraftingStatus(toggle, status, label) {
        status.innerText = `${label}: ${toggle.checked ? 'ON' : 'OFF'}`;
        status.setAttribute('aria-pressed', toggle.checked ? 'true' : 'false');
    }

    function toggleDraftingSetting(toggle, label) {
        toggle.checked = !toggle.checked;
        toggle.dispatchEvent(new Event('change'));
        showToast(`${label}: ${toggle.checked ? 'ON' : 'OFF'}`, toggle.checked ? 'success' : 'warning', 1800);
    }
    ...
    let isPdfPrintMode = false;
    let pdfPrintPreviousStatus = '';
    ...
    function normalizeTextEntities(items) {
        return items.map(entity => {
            const normalizedEntity = {
                ...entity,
                layer: typeof entity.layer === 'string' && entity.layer.trim() ? entity.layer.trim() : '0',
                locked: entity.locked === true
            };
            if (normalizedEntity.type === 'arc') {
                normalizedEntity.reversed = normalizedEntity.reversed === true;
                if (!Number.isFinite(normalizedEntity.midX) || !Number.isFinite(normalizedEntity.midY)) {
                    let sweep = normalizeAngle(normalizedEntity.endAzi - normalizedEntity.startAzi);
                    if (sweep === 0) sweep = 2 * Math.PI;
                    const midAzi = normalizeAngle(normalizedEntity.startAzi + sweep / 2);
                    normalizedEntity.midX = normalizedEntity.cx + normalizedEntity.r * Math.sin(midAzi);
                    normalizedEntity.midY = normalizedEntity.cy + normalizedEntity.r * Math.cos(midAzi);
                }
            }
            if (normalizedEntity.type === 'dxf-import' && normalizedEntity.boardType === 'title-board') {
                const normalizedBoard = { ...normalizedEntity };
            ...
            if (normalizedEntity.type !== 'text') return normalizedEntity;
    ...
    function transformImportedPoint(point, importedEntity) {
        const angle = Number(importedEntity.rotation) || 0;
        if (Math.abs(angle) < 1e-12) return { x: point.x, y: point.y };
        const center = importedEntity.rotationCenter || getEntitiesCenter(importedEntity.children || []);
    ...
    function getSnapCandidates(refPoint, excludeEntity) {
        ...
        entities.forEach(ent => {
            ...
            if (ent.type === 'dxf-import') {
                const addBoardPoint = (point, type) => snaps.push({
                    ...transformImportedPoint(point, ent),
                    type
                });
                (ent.children || []).forEach(child => {
                    getEntitySegments(child).forEach(segment => {
                        const p1 = transformImportedPoint(segment.p1, ent);
                        const p2 = transformImportedPoint(segment.p2, ent);
                        allSegments.push({ p1, p2 });
                        addBoardPoint(segment.p1, 'endpoint');
                        addBoardPoint(segment.p2, 'endpoint');
                        addBoardPoint({
                            x: (segment.p1.x + segment.p2.x) / 2,
                            y: (segment.p1.y + segment.p2.y) / 2
                        }, 'midpoint');
                    });
                    if (child.type === 'point') addBoardPoint(child, 'endpoint');
                });
                return;
            }
            ...
            if (ent.type === 'line') {
                ...
                const midpoint = getArcMidpoint(ent);
                snaps.push({ x: midpoint.x, y: midpoint.y, type: 'midpoint' });
                ...
                snaps.push(
                    { x: center.x, y: center.y, type: 'endpoint' },
    ...
    function getEntityGrips(ent) {
        ...
        if (ent.type === 'rect') {
            return [
                ...
                { id: 'm0', type: 'rect_side', x: ent.x + ent.w / 2, y: ent.y },
                { id: 'm1', type: 'rect_side', x: ent.x + ent.w, y: ent.y + ent.h / 2 },
                { id: 'm2', type: 'rect_side', x: ent.x + ent.w / 2, y: ent.y + ent.h },
                { id: 'm3', type: 'rect_side', x: ent.x, y: ent.y + ent.h / 2 },
        ...
        if (ent.type === 'arc') {
            ...
            return [
                ...
                { id: 'mid', type: 'arc_mid', label: 'Mid', color: '#00e5ff', x: midpoint.x, y: midpoint.y }
            ];
        }
        if (ent.type === 'text') {
    ...
    function getArcMidpoint(ent) {
        if (Number.isFinite(ent.midX) && Number.isFinite(ent.midY)) {
            return { x: ent.midX, y: ent.midY };
        }
        let sweep = normalizeAngle(ent.endAzi - ent.startAzi);
        if (sweep === 0) sweep = 2 * Math.PI;
        const midAzi = normalizeAngle(ent.startAzi + sweep / 2);
        return {
    ...
    function repositionArcCenter(ent, source, cx, cy) {
        const startPoint = {
            x: source.cx + source.r * Math.sin(source.startAzi),
            y: source.cy + source.r * Math.cos(source.startAzi)
        };
        const endPoint = {
            x: source.cx + source.r * Math.sin(source.endAzi),
            y: source.cy + source.r * Math.cos(source.endAzi)
        };
        ...
        if (chordLengthSquared > 1e-12) {
            ...
            const perpendicularX = -chordY;
            const perpendicularY = chordX;
            const projection = (
                (cx - midpointX) * perpendicularX +
                (cy - midpointY) * perpendicularY
            ) / chordLengthSquared;
            nextCx = midpointX + projection * perpendicularX;
            nextCy = midpointY + projection * perpendicularY;
        }

        ent.cx = nextCx;
        ent.cy = nextCy;
        ent.r = Math.max(0.001, Math.hypot(startPoint.x - nextCx, startPoint.y - nextCy));
        ent.startAzi = calculateAzimuthRad(startPoint.x - nextCx, startPoint.y - nextCy);
        ent.endAzi = calculateAzimuthRad(endPoint.x - nextCx, endPoint.y - nextCy);
        resetArcMidpoint(ent);
    }

    function getThreePointArc(startPoint, midPoint, endPoint) {
        const determinant = 2 * (
        ...
        const midSquared = midPoint.x * midPoint.x + midPoint.y * midPoint.y;
        const endSquared = endPoint.x * endPoint.x + endPoint.y * endPoint.y;
        const cx = (
            startSquared * (midPoint.y - endPoint.y) +
            midSquared * (endPoint.y - startPoint.y) +
            endSquared * (startPoint.y - midPoint.y)
        ) / determinant;
        const cy = (
            startSquared * (endPoint.x - midPoint.x) +
            midSquared * (startPoint.x - endPoint.x) +
            endSquared * (midPoint.x - startPoint.x)
        ) / determinant;
        const r = Math.hypot(startPoint.x - cx, startPoint.y - cy);
        if (!Number.isFinite(r) || r < 0.001) return null;

        let startAzi = calculateAzimuthRad(startPoint.x - cx, startPoint.y - cy);
        let endAzi = calculateAzimuthRad(endPoint.x - cx, endPoint.y - cy);
        const midAzi = calculateAzimuthRad(midPoint.x - cx, midPoint.y - cy);
        let reversed = false;
        if (!isAzimuthBetween(midAzi, startAzi, endAzi)) {
            [startAzi, endAzi] = [endAzi, startAzi];
            reversed = true;
        }

        return { cx, cy, r, startAzi, endAzi, reversed };
    }
    ...
    function applyGripModification(grip, targetPt) {
        ...
        if (ent.type === 'line') {
            ...
            if (grip.id === 'center') {
                ...
                ent.y = targetPt.y;
                ent.h = init.y + init.h - targetPt.y;
            } else if (grip.id === 'm1') {
                ent.w = targetPt.x - init.x;
            } else if (grip.id === 'm2') {
                ent.h = targetPt.y - init.y;
            } else if (grip.id === 'm3') {
                ent.x = targetPt.x;
                ent.w = init.x + init.w - targetPt.x;
            } else {
                let anchorX, anchorY;
            ...
            if (grip.id === 'center') {
                repositionArcCenter(ent, init, init.cx + dx, init.cy + dy);
                ...
                const startPoint = {
                    x: init.cx + init.r * Math.sin(init.startAzi),
                    y: init.cy + init.r * Math.cos(init.startAzi)
                };
                const endPoint = {
                    x: init.cx + init.r * Math.sin(init.endAzi),
                    y: init.cy + init.r * Math.cos(init.endAzi)
                };
                const geometry = getThreePointArc(startPoint, targetPt, endPoint);
                if (geometry) {
                    Object.assign(ent, geometry, { midX: targetPt.x, midY: targetPt.y });
                }
            }
        } else if (ent.type === 'text') {
            ...
            if (grip.id === 'start') {
                ...
                if (dimensionLength > 1e-9) {
                    const requestedOffset = (targetPt.x - ent.x1) * (-dimensionDy / dimensionLength) +
                        (targetPt.y - ent.y1) * (dimensionDx / dimensionLength);
                    ent.offset = requestedOffset;
                }
            }
        }
    ...
    function createDistanceDimension(firstPoint, secondPoint, dimensionPoint) {
        ...
        return {
            ...
            y2: secondPoint.y,
            offset,
            placement: offset < 0 ? 'below' : 'above',
    ...
    function setAngleDimensionValue(ent, value) {
        ...
        // Ray 2 stores the short geometric sweep; exterior dimensions use the
        // opposite traversal around it to represent the requested reflex angle.
        const rayDirection = ent.angleMode === 'exterior' ? -arc.direction : arc.direction;
        ent.endAzi = ent.startAzi + rayDirection * interiorSweep;
    ...
    function getDistanceDimensionPlacement(ent) {
        if (ent && (ent.placement === 'above' || ent.placement === 'below')) {
            return ent.placement;
        }
        return Number(ent && ent.offset) < 0 ? 'below' : 'above';
    }

    function setDistanceDimensionPlacement(ent, placement) {
        const side = placement === 'below' ? 'below' : 'above';
        const currentOffset = Number(ent.offset);
        const offsetMagnitude = Number.isFinite(currentOffset) ? Math.abs(currentOffset) : 0;
        ent.placement = side;
        ent.offset = side === 'below' ? -offsetMagnitude : offsetMagnitude;
    }
    ...
    function drawEntity(ent, isTemp = false) {
        ...
        if (ent.type === 'line') {
            ...
            const showName = ent.showName !== undefined ? ent.showName !== false : ent.showText !== false;
            const showZ = ent.showZ !== undefined ? ent.showZ !== false : ent.showText !== false;
            if (showName || showZ) {
                ctx.save();
                ...
                ctx.fillStyle = isSelected ? '#00bfff' : (ent.color || '#fff');
                const pointLabels = [];
                if (showName) pointLabels.push(ent.name || '');
                if (showZ) pointLabels.push(formatCoord(ent.z || 0));
                ctx.fillText(pointLabels.join(':'), p.x + 8, p.y - 8);
                ctx.restore();
            }
        }
        else if (ent.type === 'dimension' && ent.kind === 'angle') {
            ...
            if (length > 1e-9) {
                ...
                ctx.fillStyle = ent.color || '#ffd166';
                ctx.textAlign = 'center';
                const textBelowAnchor = getDistanceDimensionPlacement(ent) === 'below';
                ctx.textBaseline = textBelowAnchor ? 'top' : 'bottom';
                ctx.fillText(label, 0, textBelowAnchor ? 3 : -3);
                ctx.restore();
    ...
    function render() {
        ...
        if (isDrawing) {
            ...
            if (currentTool === 'pline' && plineVertices.length > 0) {
                ...
                if (arcDrawingStep === 1 && arcStartPoint) {
                    drawEntity({ type: 'line', x1: arcStartPoint.x, y1: arcStartPoint.y, x2: currentMouse.x, y2: currentMouse.y, color, width: 1 }, true);
                } else if (arcDrawingStep === 2 && arcStartPoint && arcMidPoint) {
                    const previewArc = getThreePointArc(arcStartPoint, arcMidPoint, currentMouse);
                    if (previewArc) {
                        drawEntity({ type: 'arc', ...previewArc, color, width }, true);
                    } else {
                        drawEntity({
                            type: 'pline',
                            points: [arcStartPoint, arcMidPoint, currentMouse],
                            closed: false,
                            color,
                            width: 1
                        }, true);
    ...
    function normalizeLayers(values) {
        ...
        values.forEach(value => {
            ...
            if (name && name.length <= 64 && !normalized.includes(name)) normalized.push(name);
        });
        return normalized;
    }

    function renderLayerOptions(selectedLayer = '0') {
        const activeLayer = layers.includes(selectedLayer) ? selectedLayer : '0';
        return layers.map(layer => `<option value="${escapeHtml(layer)}" ${layer === activeLayer ? 'selected' : ''}>${escapeHtml(layer)}</option>`).join('');
    }

    function renderLayerProperties() {
        const isOpen = localStorage.getItem('cad_layers_open') === 'true';
        return `
            <details class="prop-group" id="layers-properties-group"${isOpen ? ' open' : ''}>
                <summary class="prop-group-title" style="cursor: pointer;">Layers</summary>
                ${layers.map(layer => `
                    <div class="prop-row">
                        ${layer === '0'
                            ? '<span title="0">0</span><span style="text-align:right;color:var(--text-muted);">Default</span>'
                            : `<input type="text" class="prop-layer-name" data-layer="${escapeHtml(layer)}" maxlength="64" value="${escapeHtml(layer)}">
                               <span style="display:flex;gap:4px;">
                                   <button type="button" class="prop-layer-rename" data-layer="${escapeHtml(layer)}" title="Rename layer">Save</button>
                                   <button type="button" class="prop-layer-delete" data-layer="${escapeHtml(layer)}" title="Delete layer">Delete</button>
                               </span>`
                        }
                    </div>
                `).join('')}
                <div class="prop-row">
                    <input type="text" id="prop-new-layer-name" maxlength="64" placeholder="New layer">
                    <button type="button" id="prop-add-layer">Add</button>
                </div>
            </details>
        `;
    }

    function bindLayerProperties() {
        const layersGroup = document.getElementById('layers-properties-group');
        if (layersGroup) layersGroup.addEventListener('toggle', () => {
            localStorage.setItem('cad_layers_open', layersGroup.open ? 'true' : 'false');
        });
        const addLayerInput = document.getElementById('prop-new-layer-name');
        const addLayerButton = document.getElementById('prop-add-layer');
        const addLayer = () => {
            const name = addLayerInput ? addLayerInput.value.trim() : '';
            if (!name) {
                showToast('Enter a layer name.', 'warning', 1800);
                return;
            }
            if (layers.includes(name)) {
                showToast('This layer already exists.', 'warning', 1800);
                return;
            }
            layers.push(name);
            updatePropertiesPalette();
            triggerAutoSave();
            showToast(`Layer "${name}" added.`, 'success', 1500);
        };
        if (addLayerButton) addLayerButton.addEventListener('click', addLayer);
        if (addLayerInput) addLayerInput.addEventListener('keydown', event => {
            if (event.key === 'Enter') {
                event.preventDefault();
                addLayer();
            }
        });
        const renameLayer = input => {
            const oldName = input.dataset.layer;
            const newName = input.value.trim();
            if (!oldName || oldName === '0') return;
            if (!newName) {
                input.value = oldName;
                showToast('Layer name cannot be empty.', 'warning', 1800);
                return;
            }
            ...
            const layerIndex = layers.indexOf(oldName);
            if (layerIndex < 0) return;
            layers[layerIndex] = newName;
            entities.forEach(entity => {
                if (entity.layer === oldName) entity.layer = newName;
            });
            querySelectionCriteria.forEach(criterion => {
                if (criterion.property === 'layer' && criterion.value === oldName) criterion.value = newName;
            });
            updatePropertiesPalette();
            render();
            triggerAutoSave();
            showToast(`Layer "${oldName}" renamed to "${newName}".`, 'success', 1800);
        };
        document.querySelectorAll('.prop-layer-rename').forEach(button => {
            button.addEventListener('click', event => {
                const input = event.currentTarget.closest('.prop-row')?.querySelector('.prop-layer-name');
                if (input) renameLayer(input);
            });
        });
        document.querySelectorAll('.prop-layer-name').forEach(input => {
            input.addEventListener('keydown', event => {
                if (event.key === 'Enter') {
                    event.preventDefault();
                    renameLayer(event.currentTarget);
                }
            });
        });
        document.querySelectorAll('.prop-layer-delete').forEach(button => {
            button.addEventListener('click', event => {
                const name = event.currentTarget.dataset.layer;
                if (!name || name === '0') return;
                if (!window.confirm(`Delete layer "${name}"? Its entities will move to layer "0".`)) return;
                entities.forEach(entity => {
                    if (entity.layer === name) entity.layer = '0';
                });
                layers = layers.filter(layer => layer !== name);
                updatePropertiesPalette();
                render();
                triggerAutoSave();
                showToast(`Layer "${name}" deleted.`, 'success', 1500);
            });
        });
    }

    function collectQueryableProperties(value, prefix = '', properties = new Set()) {
        if (!value || typeof value !== 'object' || Array.isArray(value)) return properties;
        Object.entries(value).forEach(([key, child]) => {
            const path = prefix ? `${prefix}.${key}` : key;
            if (child !== null && typeof child === 'object' && !Array.isArray(child)) {
                collectQueryableProperties(child, path, properties);
            } else {
                properties.add(path);
            }
        });
    ...
    function getQueryableEntityTypes() {
        const labels = {
            line: 'Γραμμή',
            pline: 'Πολυγραμμή',
            rect: 'Ορθογώνιο',
            ...
            text: 'Κείμενο',
            dimension: 'Διάσταση',
            'dxf-import': 'Εισαγόμενο DXF'
        };
        return [...new Set(entities.map(entity => entity.type).filter(Boolean))]
            .sort((a, b) => (labels[a] || a).localeCompare(labels[b] || b))
            .map(type => [type, labels[type] || type]);
    }

    function getQueryableProperties(entityType) {
        const properties = new Set();
        entities
            .filter(entity => entity.type === entityType)
            .forEach(entity => collectQueryableProperties(entity, '', properties));
        properties.delete('type');
        ['layer', 'color', 'width', 'locked'].forEach(property => properties.add(property));
        const standardProperties = {
            point: ['pointType', 'name', 'showName', 'showZ', 'locked', 'x', 'y', 'z'],
            line: ['x1', 'y1', 'x2', 'y2', 'deltaX', 'deltaY', 'length', 'azimuth'],
            pline: ['points', 'closed', 'elevation'],
            rect: ['x', 'y', 'w', 'h'],
            circle: ['cx', 'cy', 'r'],
            arc: ['cx', 'cy', 'r', 'startAzi', 'endAzi', 'reversed'],
            ellipse: ['cx', 'cy', 'rx', 'ry'],
            text: ['text', 'textMode', 'fontFamily', 'justify', 'height', 'rotation', 'x', 'y'],
            dimension: ['kind', 'decimals', 'locked', 'angleMode', 'placement']
        };
        (standardProperties[entityType] || []).forEach(property => properties.add(property));
        return [...properties].sort((a, b) => a.localeCompare(b));
    }

    function getPropertyValue(entity, path) {
        if (path === 'locked') return entity.locked === true;
        if (entity.type === 'line') {
            const deltaX = Number(entity.x2) - Number(entity.x1);
            const deltaY = Number(entity.y2) - Number(entity.y1);
            if (path === 'deltaX') return deltaX;
            if (path === 'deltaY') return deltaY;
            if (path === 'length') return Math.hypot(deltaX, deltaY);
            if (path === 'azimuth') return azimuthRadToValue(calculateAzimuthRad(deltaX, deltaY));
        }
    ...
    function getQueryValueOptions(entityType, property) {
        if (property === 'layer') return layers.map(layer => [layer, layer]);
        if (property === 'pointType' && entityType === 'point') return pointTypes;
        if (['locked', 'closed', 'showName', 'showZ', 'showText', 'reversed'].includes(property)) {
            return [['true', 'Yes'], ['false', 'No']];
        }
        const optionsByProperty = {
            width: [1, 2, 3, 4, 5].map(value => [String(value), `${value} px`]),
            decimals: [0, 1, 2, 3, 4, 5, 6].map(value => [String(value), String(value)]),
            kind: [['distance', 'Distance'], ['angle', 'Angle']],
            angleMode: [['interior', 'Interior'], ['exterior', 'Exterior']],
            placement: [['above', 'Above'], ['below', 'Below']],
            textMode: [['one-line', 'One line'], ['multiline', 'Multiline']],
            justify: [['left', 'Left'], ['center', 'Center'], ['right', 'Right']],
            fontFamily: ['Arial', 'Arial Narrow', 'Tahoma', 'Verdana', 'Consolas', 'Courier New'].map(value => [value, value]),
            boardType: [['title-board', 'Title board']],
            'hatch.pattern': [['diagonal', 'Diagonal']],
            'hatch.sideSign': [['-1', 'Left / Inside'], ['1', 'Right / Outside']]
        };
        return optionsByProperty[property] || null;
    }

    function queryValueMatches(actualValue, operator, expectedText) {
        if (operator === 'exists') return actualValue !== undefined && actualValue !== null;
        if (operator === 'not-exists') return actualValue === undefined || actualValue === null;
        if (actualValue === undefined || actualValue === null) return operator === 'not-equals';

        const actualNumber = typeof actualValue === 'number' ? actualValue : Number(actualValue);
        const expectedNumber = Number(expectedText);
        const numericComparison = Number.isFinite(actualNumber) && expectedText.trim() !== '' && Number.isFinite(expectedNumber);
        if (['greater', 'greater-equal', 'less', 'less-equal'].includes(operator)) {
            if (!numericComparison) return false;
            if (operator === 'greater') return actualNumber > expectedNumber;
            if (operator === 'greater-equal') return actualNumber >= expectedNumber;
            if (operator === 'less') return actualNumber < expectedNumber;
            return actualNumber <= expectedNumber;
        }

        const actualText = typeof actualValue === 'object'
        ...
        if (operator === 'not-contains') return !normalizedActual.includes(normalizedExpected);
        if (operator === 'equals') {
            return numericComparison ? actualNumber === expectedNumber : normalizedActual === normalizedExpected;
        }
    ...
    function renderQuerySelectionProperties() {
        const entityTypes = getQueryableEntityTypes();
        const propertyLabels = {
            deltaX: 'Delta X',
            deltaY: 'Delta Y',
            length: 'Length',
            azimuth: `Azimuth (${getAngleUnitLabel()})`
        };
        const isOpen = localStorage.getItem('cad_query_selection_open') === 'true';
        const operators = [
            ['equals', 'Equals'],
        ...
        return `
            <details class="prop-group" id="query-selection-group"${isOpen ? ' open' : ''}>
                <summary class="prop-group-title" style="cursor:pointer;">Query Selection</summary>
                ${querySelectionCriteria.map((criterion, index) => {
                    const entityType = entityTypes.some(([type]) => type === criterion.entityType)
                        ? criterion.entityType
                        : (entityTypes[0]?.[0] || '');
                    const properties = getQueryableProperties(entityType);
                    const property = properties.includes(criterion.property)
                        ? criterion.property
                        : (properties[0] || '');
                    criterion.entityType = entityType;
                    criterion.property = property;
                    const valueOptions = criterion.operator === 'equals'
                        ? getQueryValueOptions(entityType, property)
                        : null;
                    if (valueOptions && !valueOptions.some(([value]) => String(value) === String(criterion.value))) {
                        criterion.value = valueOptions[0]?.[0] || '';
                    }
                    return `
                    <div style="border-bottom:1px solid var(--border-color);padding:5px 0;">
                        <div class="prop-row">
                            <label>Object</label>
                            <select class="query-entity-type" data-query-index="${index}">
                                ${entityTypes.map(([type, label]) => `<option value="${escapeHtml(type)}" ${type === entityType ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('')}
                            </select>
                        </div>
                        <div class="prop-row">
                            <label>Property</label>
                            <select class="query-property" data-query-index="${index}">
                                ${properties.map(option => `<option value="${escapeHtml(option)}" ${option === property ? 'selected' : ''}>${escapeHtml(propertyLabels[option] || option)}</option>`).join('')}
                            </select>
                        </div>
                        <div class="prop-row">
                            <label>Operator</label>
                            <select class="query-operator" data-query-index="${index}">
                                ${operators.map(([value, label]) => `<option value="${value}" ${value === criterion.operator ? 'selected' : ''}>${label}</option>`).join('')}
                            </select>
                        </div>
                        <div class="prop-row">
                            <label>Value</label>
                            ${valueOptions
                                ? `<select class="query-value" data-query-index="${index}">
                                    ${valueOptions.map(([value, label]) => `<option value="${escapeHtml(value)}" ${String(value) === String(criterion.value) ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('')}
                                </select>`
                                : `<input type="text" class="query-value" data-query-index="${index}" value="${escapeHtml(criterion.value)}" placeholder="Value" ${['exists', 'not-exists'].includes(criterion.operator) ? 'disabled' : ''}>`
                            }
                        </div>
                        <div style="display:flex;justify-content:flex-end;">
                            <button type="button" class="query-remove" data-query-index="${index}" title="Remove criterion">×</button>
                        </div>
                    </div>
                `; }).join('')}
                <div style="display:flex;gap:5px;margin-top:6px;">
                    <button type="button" id="query-add-criterion" ${entityTypes.length ? '' : 'disabled'}>Add criterion</button>
                    <button type="button" id="query-run" ${querySelectionCriteria.length ? '' : 'disabled'}>Select</button>
                    <button type="button" id="query-clear" ${querySelectionCriteria.length ? '' : 'disabled'}>Clear</button>
                </div>
                <div style="margin-top:5px;color:var(--text-muted);font-size:10px;">All criteria must match (AND).</div>
            </details>
        `;
    }

    function bindQuerySelectionProperties() {
        const queryGroup = document.getElementById('query-selection-group');
        if (queryGroup) queryGroup.addEventListener('toggle', () => {
            localStorage.setItem('cad_query_selection_open', queryGroup.open ? 'true' : 'false');
        });
        document.querySelectorAll('.query-entity-type').forEach(input => {
            input.addEventListener('change', event => {
                const criterion = querySelectionCriteria[Number(event.target.dataset.queryIndex)];
                criterion.entityType = event.target.value;
                criterion.property = getQueryableProperties(criterion.entityType)[0] || '';
                updatePropertiesPalette();
            });
        });
        document.querySelectorAll('.query-property').forEach(input => {
            input.addEventListener('change', event => {
                const criterion = querySelectionCriteria[Number(event.target.dataset.queryIndex)];
                criterion.property = event.target.value;
                criterion.value = '';
                updatePropertiesPalette();
            });
        });
        document.querySelectorAll('.query-operator').forEach(input => {
            input.addEventListener('change', event => {
                querySelectionCriteria[Number(event.target.dataset.queryIndex)].operator = event.target.value;
                updatePropertiesPalette();
            });
        });
        document.querySelectorAll('.query-value').forEach(input => {
            const updateValue = event => {
                querySelectionCriteria[Number(event.target.dataset.queryIndex)].value = event.target.value;
            };
            input.addEventListener('input', updateValue);
            input.addEventListener('change', updateValue);
        });
        document.querySelectorAll('.query-remove').forEach(button => {
            button.addEventListener('click', event => {
                querySelectionCriteria.splice(Number(event.currentTarget.dataset.queryIndex), 1);
                updatePropertiesPalette();
            });
        });
        const addButton = document.getElementById('query-add-criterion');
        if (addButton) addButton.addEventListener('click', () => {
            const entityType = getQueryableEntityTypes()[0]?.[0];
            if (!entityType) return;
            const property = getQueryableProperties(entityType)[0] || '';
            querySelectionCriteria.push({ entityType, property, operator: 'equals', value: '' });
            updatePropertiesPalette();
        });
        const clearButton = document.getElementById('query-clear');
        ...
        if (runButton) runButton.addEventListener('click', () => {
            const matches = entities.filter(entity => querySelectionCriteria.every(criterion =>
                entity.type === criterion.entityType &&
                queryValueMatches(getPropertyValue(entity, criterion.property), criterion.operator, criterion.value)
            ));
            selectedHatch = null;
            selectedEntities = new Set(matches);
            selectedEntity = matches[0] || null;
            selectedSegmentIndex = null;
            selectedVertexIndex = 0;
            updatePropertiesPalette();
            render();
            showToast(`${matches.length} object(s) selected.`, matches.length ? 'success' : 'warning', 1800);
        });
    }

    function getCommonPrimitiveProperties(selectedItems) {
        if (selectedItems.length < 2) return [];
        const excluded = new Set(['type', 'color', 'width', 'layer', 'locked', 'kind', 'boardType', 'generatedBy', 'rotationUnit']);
        return Object.keys(selectedItems[0])
            .filter(property => !excluded.has(property))
            .filter(property => selectedItems.every(entity =>
                Object.prototype.hasOwnProperty.call(entity, property) &&
                (entity[property] === null || typeof entity[property] !== 'object')
            ))
            .sort((a, b) => a.localeCompare(b));
    }

    function renderCommonSelectionProperties(selectedItems) {
        const entityType = selectedItems.every(entity => entity.type === selectedItems[0].type)
        ...
        const properties = getCommonPrimitiveProperties(selectedItems);
        if (!properties.length) return '';
        return `
            <div class="prop-group">
                <div class="prop-group-title">Common Properties</div>
                ${properties.map(property => {
                    const values = selectedItems.map(entity => entity[property]);
                    const firstValue = values[0];
                    const sameValue = values.every(value => Object.is(value, firstValue));
                    const valueOptions = getQueryValueOptions(entityType, property);
                    if (typeof firstValue === 'boolean') {
                        return `<div class="prop-row"><label>${escapeHtml(labels[property] || property)}</label><input type="checkbox" class="prop-common-value" data-property="${escapeHtml(property)}" ${sameValue && firstValue ? 'checked' : ''} ${sameValue ? '' : 'data-mixed="true"'}></div>`;
                    }
                    if (valueOptions) {
                        return `<div class="prop-row"><label>${escapeHtml(labels[property] || property)}</label><select class="prop-common-value" data-property="${escapeHtml(property)}">
                            ${sameValue ? '' : '<option value="" selected disabled>— Mixed —</option>'}
                            ${valueOptions.map(([value, label]) => `<option value="${escapeHtml(value)}" ${sameValue && String(value) === String(firstValue) ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('')}
                        </select></div>`;
                    }
                    return `<div class="prop-row"><label>${escapeHtml(labels[property] || property)}</label><input type="text" class="prop-common-value" data-property="${escapeHtml(property)}" value="${sameValue ? escapeHtml(firstValue) : ''}" placeholder="${sameValue ? '' : 'Mixed'}"></div>`;
                }).join('')}
            </div>
        `;
    }

    function bindCommonSelectionProperties(selectedItems) {
        document.querySelectorAll('.prop-common-value').forEach(input => {
            if (input.dataset.mixed === 'true' && input.type === 'checkbox') input.indeterminate = true;
            input.addEventListener('change', event => {
                const property = event.target.dataset.property;
                const currentValue = selectedItems[0][property];
                let value = event.target.type === 'checkbox' ? event.target.checked : event.target.value;
                if (typeof currentValue === 'number') {
                    value = parseStrictFloat(value, NaN);
                    if (!Number.isFinite(value)) {
                        updatePropertiesPalette();
                        showToast(`${property} must be a number.`, 'error', 1800);
                        return;
                    }
                }
                if (typeof currentValue === 'boolean' && event.target.type !== 'checkbox') value = value === 'true';
                saveState();
                selectedItems.forEach(entity => { entity[property] = value; });
                updatePropertiesPalette();
                render();
                triggerAutoSave();
                showToast(`${selectedItems.length} entities updated.`, 'success', 1500);
            });
        });
    }
    ...
    function renderHatchProperties(entity) {
        ...
        propContainer.innerHTML = `
            ...
            ${renderQuerySelectionProperties()}
            <div class="prop-group">
    ...
    function updatePropertiesPalette() {
        let html = `
            <details class="prop-group" id="user-properties-group">
                <summary class="prop-group-title" style="cursor: pointer;">User</summary>
                <div class="prop-row">
                    <label for="prop-username">Username</label>
                    <input type="text" id="prop-username" maxlength="24" value="">
                </div>
            </details>
            <details class="prop-group" id="paperspace-properties-group">
                <summary class="prop-group-title" style="cursor: pointer;">Paperspace</summary>
                <div class="prop-row">
                    <label for="prop-paper-size">Paper size</label>
                    <select id="prop-paper-size">
                        ${[...paperSizeSelect.options].map(option => `<option value="${option.value}" ${option.value === paperSizeSelect.value ? 'selected' : ''}>${option.textContent}</option>`).join('')}
                    </select>
                </div>
                <div class="prop-row">
                    <label for="prop-print-scale">Scale</label>
                    <select id="prop-print-scale">
        ...
        const selectedItems = selectedEntities.size > 1 ? [...selectedEntities] : [selectedEntity];
        const sameColor = selectedItems.every(entity => (entity.color || '#ffffff') === (selectedItems[0].color || '#ffffff'));
        const sameWidth = selectedItems.every(entity => Number(entity.width || 1) === Number(selectedItems[0].width || 1));
        const sameLayer = selectedItems.every(entity => (entity.layer || '0') === (selectedItems[0].layer || '0'));
        const sameLocked = selectedItems.every(entity => Boolean(entity.locked) === Boolean(selectedItems[0].locked));
        html += `
            <div class="prop-group">
                <div class="prop-group-title">General</div>
                <div class="prop-row">
                    <label>Color${sameColor ? '' : ' (Mixed)'}</label>
                    <input type="color" id="prop-color" value="${selectedEntity.color || '#ffffff'}">
                </div>
                <div class="prop-row">
                    <label>Line Weight</label>
                    <select id="prop-width">
                        ${sameWidth ? '' : '<option value="" selected disabled>— Mixed —</option>'}
                        ${[1, 2, 3, 4, 5].map(w => `<option value="${w}" ${sameWidth && selectedEntity.width == w ? 'selected' : ''}>${w} px</option>`).join('')}
                    </select>
                </div>
                <div class="prop-row">
                    <label for="prop-layer">Layer</label>
                    <select id="prop-layer">
                        ${sameLayer ? '' : '<option value="" selected disabled>— Mixed —</option>'}
                        ${layers.map(layer => `<option value="${escapeHtml(layer)}" ${sameLayer && layer === (selectedEntity.layer || '0') ? 'selected' : ''}>${escapeHtml(layer)}</option>`).join('')}
                    </select>
                </div>
                <div class="prop-row">
                    <label for="prop-locked">Locked</label>
                    <input type="checkbox" id="prop-locked" ${sameLocked && selectedEntity.locked ? 'checked' : ''} ${sameLocked ? '' : 'data-mixed="true"'}>
                </div>
            </div>
        `;
        if (selectedItems.length > 1) {
            html += renderCommonSelectionProperties(selectedItems);
        } else {
        ...
        if (selectedEntity.type === 'line') {
            ...
            html += `
                <div class="prop-group">
                    ...
                    <div class="prop-row"><label for="prop-arc-reversed">Reversed</label><input type="checkbox" id="prop-arc-reversed" ${selectedEntity.reversed ? 'checked' : ''}></div>
            ...
            if (selectedEntity.kind === 'angle') {
                ...
                html += `
                    <div class="prop-group">
                        ...
                        <div class="prop-row"><label>Placement</label><select id="prop-dimension-placement">
                            <option value="above" ${getDistanceDimensionPlacement(selectedEntity) === 'above' ? 'selected' : ''}>Above</option>
                            <option value="below" ${getDistanceDimensionPlacement(selectedEntity) === 'below' ? 'selected' : ''}>Below</option>
                        </select></div>
            ...
            const pointShowName = selectedEntity.showName !== undefined
                ? selectedEntity.showName !== false
                : selectedEntity.showText !== false;
            const pointShowZ = selectedEntity.showZ !== undefined
                ? selectedEntity.showZ !== false
                : selectedEntity.showText !== false;
            const pointType = pointTypes.some(([value]) => value === selectedEntity.pointType)
                ? selectedEntity.pointType
                : 'simple';
            html += `
                <div class="prop-group">
                    <div class="prop-group-title">Point Geometry</div>
                    <div class="prop-row"><label for="prop-point-type">Type</label><select id="prop-point-type">
                        ${pointTypes.map(([value, label]) => `<option value="${value}" ${value === pointType ? 'selected' : ''}>${label}</option>`).join('')}
                    </select></div>
                    <div class="prop-row"><label>Name</label><input type="text" id="prop-point-name" value="${selectedEntity.name || ''}"></div>
                    <div class="prop-row"><label>Show Name</label><input type="checkbox" id="prop-point-show-name" ${pointShowName ? 'checked' : ''}></div>
                    <div class="prop-row"><label>Show Z</label><input type="checkbox" id="prop-point-show-z" ${pointShowZ ? 'checked' : ''}></div>
        ...
        bindOsnapProperties();
        bindPaperspaceProperties();
        bindCommonSelectionProperties(selectedItems);
        ...
        if (colorInput) colorInput.addEventListener('change', (e) => {
            saveState();
            selectedItems.forEach(entity => { entity.color = e.target.value; });
            updatePropertiesPalette();
            render(); 
            triggerAutoSave();
            showToast(`${selectedItems.length > 1 ? `${selectedItems.length} entities` : 'Entity'} color updated.`, 'success', 1500);
        });

        const widthSelect = document.getElementById('prop-width');
        if (widthSelect) widthSelect.addEventListener('change', (e) => { 
            saveState(); 
            selectedItems.forEach(entity => { entity.width = parseInt(e.target.value); });
            updatePropertiesPalette();
            render(); 
            triggerAutoSave();
            showToast(`${selectedItems.length > 1 ? `${selectedItems.length} entities` : 'Entity'} line width updated.`, 'success', 1500);
        });

        const layerSelect = document.getElementById('prop-layer');
        if (layerSelect) layerSelect.addEventListener('change', (e) => {
            saveState();
            const targets = selectedEntities.size > 1 ? [...selectedEntities] : [selectedEntity];
            targets.forEach(entity => { entity.layer = e.target.value; });
            render();
            triggerAutoSave();
            showToast(`${targets.length > 1 ? `${targets.length} entities` : 'Entity'} moved to layer "${e.target.value}".`, 'success', 1500);
        });

        const lockedInput = document.getElementById('prop-locked');
        if (lockedInput) {
            if (lockedInput.dataset.mixed === 'true') lockedInput.indeterminate = true;
            lockedInput.addEventListener('change', (e) => {
                saveState();
                selectedItems.forEach(entity => { entity.locked = e.target.checked; });
                activeMove = null;
                activeGrip = null;
                updatePropertiesPalette();
                render();
                triggerAutoSave();
                showToast(`${selectedItems.length > 1 ? `${selectedItems.length} entities` : 'Entity'} ${e.target.checked ? 'locked' : 'unlocked'}.`, 'success', 1500);
            });
        }
        ...
        if (selectedEntity.type === 'dimension') {
            if (selectedEntity.kind === 'angle') {
                ...
                if (dimensionOffsetInput) dimensionOffsetInput.addEventListener('change', (e) => {
                    ...
                    selectedEntity.placement = newOffset < 0 ? 'below' : 'above';
                ...
                const dimensionPlacementSelect = document.getElementById('prop-dimension-placement');
                if (dimensionPlacementSelect) dimensionPlacementSelect.addEventListener('change', (e) => {
                    saveState();
                    setDistanceDimensionPlacement(selectedEntity, e.target.value);
                    const dx = selectedEntity.x2 - selectedEntity.x1;
                    const dy = selectedEntity.y2 - selectedEntity.y1;
                    const currentDistance = Math.hypot(dx, dy);
                    if (currentDistance > 1e-9) {
                        selectedEntity.textX = (selectedEntity.x1 + selectedEntity.x2) / 2 - dy / currentDistance * selectedEntity.offset;
                        selectedEntity.textY = (selectedEntity.y1 + selectedEntity.y2) / 2 + dx / currentDistance * selectedEntity.offset;
                    }
                    updatePropertiesPalette();
                    render();
                    triggerAutoSave();
                    showToast('Dimension placement updated.', 'success', 1500);
                });
            ...
            bindInput('prop-arc-cx', v => {
                const source = { ...selectedEntity };
                repositionArcCenter(selectedEntity, source, v, source.cy);
                updatePropertiesPalette();
            });
            bindInput('prop-arc-cy', v => {
                const source = { ...selectedEntity };
                repositionArcCenter(selectedEntity, source, source.cx, v);
                updatePropertiesPalette();
            });
            bindInput('prop-arc-r', v => {
                selectedEntity.r = Math.max(0.001, v);
                resetArcMidpoint(selectedEntity);
            });
            bindInput('prop-arc-start', v => {
                selectedEntity.startAzi = azimuthValueToRad(v);
                resetArcMidpoint(selectedEntity);
                updatePropertiesPalette();
            });
            bindInput('prop-arc-end', v => {
                selectedEntity.endAzi = azimuthValueToRad(v);
                resetArcMidpoint(selectedEntity);
                updatePropertiesPalette();
            });
            const arcReversedInput = document.getElementById('prop-arc-reversed');
            if (arcReversedInput) arcReversedInput.addEventListener('change', event => {
                saveState();
                [selectedEntity.startAzi, selectedEntity.endAzi] = [selectedEntity.endAzi, selectedEntity.startAzi];
                selectedEntity.reversed = event.target.checked;
                resetArcMidpoint(selectedEntity);
                updatePropertiesPalette();
                render();
                triggerAutoSave();
                showToast(`Arc direction ${selectedEntity.reversed ? 'reversed' : 'restored'}.`, 'success', 1500);
            });
        } else if (selectedEntity.type === 'point') {
            const pointTypeInput = document.getElementById('prop-point-type');
            if (pointTypeInput) pointTypeInput.addEventListener('change', (e) => {
                saveState();
                selectedEntity.pointType = e.target.value;
                updatePropertiesPalette();
                render();
                triggerAutoSave();
                showToast('Point type updated.', 'success', 1500);
            });
            ...
            const pointShowNameInput = document.getElementById('prop-point-show-name');
            if (pointShowNameInput) pointShowNameInput.addEventListener('change', (e) => {
                saveState();
                selectedEntity.showName = e.target.checked;
                updatePropertiesPalette();
                render();
                showToast(`Point name ${selectedEntity.showName ? 'shown' : 'hidden'}.`, 'success', 1500);
            });
            const pointShowZInput = document.getElementById('prop-point-show-z');
            if (pointShowZInput) pointShowZInput.addEventListener('change', (e) => {
                saveState();
                selectedEntity.showZ = e.target.checked;
                updatePropertiesPalette();
                render();
                showToast(`Point Z ${selectedEntity.showZ ? 'shown' : 'hidden'}.`, 'success', 1500);
            });
    ...
    function bindPaperspaceProperties() {
        const paperSizeInput = document.getElementById('prop-paper-size');
        if (paperSizeInput) paperSizeInput.addEventListener('change', event => {
            paperSizeSelect.value = event.target.value;
            localStorage.setItem('cad_paper_size', paperSizeSelect.value);
            render();
        });
        const printScaleInput = document.getElementById('prop-print-scale');
        if (printScaleInput) printScaleInput.addEventListener('change', event => {
            printScaleSelect.value = event.target.value;
        ...
        if (angleUnitInput) angleUnitInput.addEventListener('change', event => {
            ...
            const paperspaceGroup = document.getElementById('paperspace-properties-group');
            if (paperspaceGroup) paperspaceGroup.open = true;
        });
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (e.button === 0 && currentTool === 'select' && !moveCommand && !activeGrip &&
            !isImageCaptureMode && !isPdfPrintMode) {
        ...
        if (e.button === 0 && (isImageCaptureMode || isPdfPrintMode)) {
            const captureSnap = findBestSnap(mouseScreen.x, mouseScreen.y, null, {});
            const capturePoint = captureSnap
                ? { x: captureSnap.screenX, y: captureSnap.screenY }
                : mouseScreen;
            imageCaptureSelection = { start: capturePoint, current: capturePoint };
            canvas.style.cursor = 'crosshair';
            render();
            return;
        }
        ...
        if (e.button === 0) {
            ...
            if (currentTool === 'select') {
                ...
                if (selectedEntity && selectedEntities.size <= 1 && !selectedEntity.locked) {
                    const gripHit = hitTestGrip(mouseScreen.x, mouseScreen.y, selectedEntity);
                    if (gripHit) {
                        saveState();
                        activeGrip = {
                            ...gripHit,
                            startWorld: { x: gripHit.x, y: gripHit.y },
                ...
                if (angleGripHit) {
                    ...
                    activeGrip = {
                        ...angleGripHit,
                        startWorld: { x: angleGripHit.x, y: angleGripHit.y },
                ...
                if (hit) {
                    ...
                    if (selectedEntity && selectedEntities.has(hit.entity) && !selectedEntity.locked) {
                        activeMove = {
            ...
            if (currentTool === 'arc') {
                if (arcDrawingStep === 0) {
                    ...
                    arcDrawingStep = 1;
                    isDrawing = true;
                    showToast('Arc: Click a point on the arc.', 'info', 3000);
                } else if (arcDrawingStep === 1) {
                    arcMidPoint = effectiveCoords;
                    arcDrawingStep = 2;
                    showToast('Arc: Click the second endpoint.', 'info', 3000);
                } else if (arcDrawingStep === 2) {
                    const arcGeometry = getThreePointArc(arcStartPoint, arcMidPoint, effectiveCoords);
                    if (!arcGeometry) {
                        showToast('The three arc points must be distinct and non-collinear.', 'error', 3000);
                        return;
                    }
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        if (imageCaptureSelection) {
            const captureSnap = findBestSnap(sx, sy, null, {});
            activeSnap = captureSnap;
            const capturePoint = captureSnap
                ? { x: captureSnap.screenX, y: captureSnap.screenY }
                : { x: sx, y: sy };
            imageCaptureSelection.current = {
                x: Math.max(0, Math.min(canvas.width, capturePoint.x)),
                y: Math.max(0, Math.min(canvas.height, capturePoint.y))
            };
            render();
            return;
        }
        ...
        if (!activeGrip && currentTool === 'select') {
            const selectedEntityLocked = selectedEntity && selectedEntity.locked;
            hoveredGrip = selectedEntity && !selectedEntityLocked
                ? hitTestGrip(sx, sy, selectedEntity)
                : (!selectedEntity ? hitTestAngleDimensionGrip(sx, sy) : null);
    ...
    window.addEventListener('mouseup', (e) => {
        if (imageCaptureSelection && e.button === 0) {
            ...
            const captureSnap = findBestSnap(endScreen.x, endScreen.y, null, {});
            const capturePoint = captureSnap
                ? { x: captureSnap.screenX, y: captureSnap.screenY }
                : endScreen;
            selection.current = {
            ...
            const wasPdfPrint = isPdfPrintMode;
            isImageCaptureMode = false;
            isPdfPrintMode = false;
            canvas.style.cursor = 'default';
            statusMode.innerText = wasPdfPrint ? pdfPrintPreviousStatus : imageCapturePreviousStatus;
            render();
            if (wasPdfPrint) {
                printCanvasRegionAsPdf(selection.start, selection.current);
            } else {
    ...
    function copyCanvasRegionAsJpg(start, end) {
        ...
        const dataUrlToBlob = (url, type) => {
            const base64 = url.split(',')[1];
            const bytes = Uint8Array.from(atob(base64), character => character.charCodeAt(0));
            return new Blob([bytes], { type });
        };
        const jpgBlob = dataUrlToBlob(dataUrl, 'image/jpeg');
        ...
        // Chromium clipboard image writes support PNG reliably, while image/jpeg is commonly rejected.
        const pngBlob = dataUrlToBlob(exportCanvas.toDataURL('image/png'), 'image/png');
        navigator.clipboard.write([new ClipboardItem({ 'image/png': pngBlob })])
            .then(() => showToast('Selection copied to clipboard.', 'success', 1800))
            .catch(error => {
                console.error('Clipboard image write failed:', error);
                downloadFallback();
            });
    }
    ...
    function printCanvasRegionAsPdf(start, end) {
        const sourceX = Math.max(0, Math.min(start.x, end.x));
        const sourceY = Math.max(0, Math.min(start.y, end.y));
        const sourceRight = Math.min(canvas.width, Math.max(start.x, end.x));
        const sourceBottom = Math.min(canvas.height, Math.max(start.y, end.y));
        ...
        const height = Math.floor(sourceBottom - sourceY);
        if (width < 2 || height < 2) {
            showToast('The print area is too small.', 'warning', 1800);
            return;
        }

        const topLeftWorld = screenToWorld(sourceX, sourceY);
        const bottomRightWorld = screenToWorld(sourceRight, sourceBottom);
        const worldWidth = Math.abs(bottomRightWorld.x - topLeftWorld.x);
        const worldHeight = Math.abs(bottomRightWorld.y - topLeftWorld.y);
        const scale = Math.max(1, Number(printScaleSelect.value) || 100);
        const widthMm = worldWidth * 1000 / scale;
        const heightMm = worldHeight * 1000 / scale;
        const left = Math.min(topLeftWorld.x, bottomRightWorld.x);
        const right = Math.max(topLeftWorld.x, bottomRightWorld.x);
        const bottom = Math.min(topLeftWorld.y, bottomRightWorld.y);
        const top = Math.max(topLeftWorld.y, bottomRightWorld.y);
        const escapeXml = value => String(value ?? '')
            .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
            .replace(/"/g, '&quot;').replace(/'/g, '&apos;');
        const colorOf = entity => escapeXml(entity.color || '#ffffff');
        const widthOf = entity => Math.max(0.15, Number(entity.width) || 1);
        const y = value => top + bottom - value;
        const point = (xValue, yValue) => `${Number(xValue).toFixed(6)},${y(yValue).toFixed(6)}`;
        const svgEntities = [];
        const addSvgEntity = entity => {
            if (!entity) return;
            const color = colorOf(entity);
            const stroke = `stroke="${color}" stroke-width="${widthOf(entity)}" fill="none"`;
            if (entity.type === 'line') {
                svgEntities.push(`<line x1="${entity.x1}" y1="${y(entity.y1)}" x2="${entity.x2}" y2="${y(entity.y2)}" ${stroke}/>`);
            } else if (entity.type === 'rect') {
                ...
                svgEntities.push(`<rect x="${x1}" y="${y(y2)}" width="${x2 - x1}" height="${y2 - y1}" ${stroke}/>`);
            } else if (entity.type === 'pline' && Array.isArray(entity.points)) {
                const points = entity.points.map(item => point(item.x, item.y)).join(' ');
                svgEntities.push(`<polyline points="${points}" ${stroke}${entity.closed ? ' fill="none"' : ''}/>`);
            } else if (entity.type === 'circle') {
                svgEntities.push(`<circle cx="${entity.cx}" cy="${y(entity.cy)}" r="${Math.abs(entity.r)}" ${stroke}/>`);
            } else if (entity.type === 'ellipse') {
                svgEntities.push(`<ellipse cx="${entity.cx}" cy="${y(entity.cy)}" rx="${Math.abs(entity.rx)}" ry="${Math.abs(entity.ry)}" ${stroke}/>`);
            } else if (entity.type === 'arc') {
                const startX = entity.cx + entity.r * Math.sin(entity.startAzi);
                const startY = entity.cy + entity.r * Math.cos(entity.startAzi);
                const endX = entity.cx + entity.r * Math.sin(entity.endAzi);
                const endY = entity.cy + entity.r * Math.cos(entity.endAzi);
                const sweep = ((entity.endAzi - entity.startAzi) % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2);
                svgEntities.push(`<path d="M ${startX} ${y(startY)} A ${Math.abs(entity.r)} ${Math.abs(entity.r)} 0 ${sweep > Math.PI ? 1 : 0} 0 ${endX} ${y(endY)}" ${stroke}/>`);
            } else if (entity.type === 'point') {
                const radius = Math.max(0.5, widthOf(entity) * 0.8);
                svgEntities.push(`<circle cx="${entity.x}" cy="${y(entity.y)}" r="${radius}" fill="${color}"/>`);
                if (entity.showName !== false && entity.name) {
                    svgEntities.push(`<text x="${entity.x + radius * 2}" y="${y(entity.y) - radius * 2}" fill="${color}" font-size="${Math.max(1, widthOf(entity) * 6)}">${escapeXml(entity.name)}</text>`);
                }
                if (entity.showZ !== false) {
                    svgEntities.push(`<text x="${entity.x + radius * 2}" y="${y(entity.y) + radius * 6}" fill="${color}" font-size="${Math.max(1, widthOf(entity) * 6)}">${escapeXml(formatCoord(entity.z || 0))}</text>`);
                }
            } else if (entity.type === 'text') {
                const fontSize = Math.max(0.5, Number(entity.height) || 1);
                svgEntities.push(`<text x="${entity.x}" y="${y(entity.y)}" fill="${color}" font-size="${fontSize}" transform="rotate(${-(Number(entity.rotation) || 0)} ${entity.x} ${y(entity.y)})">${escapeXml(entity.text || '')}</text>`);
            } else if (entity.type === 'dimension' && entity.kind !== 'angle') {
                const dx = entity.x2 - entity.x1;
                const dy = entity.y2 - entity.y1;
                const length = Math.hypot(dx, dy);
                if (length > 1e-9) {
                    ...
                    svgEntities.push(`<line x1="${d1.x}" y1="${y(d1.y)}" x2="${d2.x}" y2="${y(d2.y)}" ${stroke}/><line x1="${entity.x1}" y1="${y(entity.y1)}" x2="${d1.x}" y2="${y(d1.y)}" ${stroke}/><line x1="${entity.x2}" y1="${y(entity.y2)}" x2="${d2.x}" y2="${y(d2.y)}" ${stroke}/>`);
                    const text = getDimensionTextPosition(entity);
                    svgEntities.push(`<text x="${text.x}" y="${y(text.y)}" fill="${color}" font-size="${Math.max(0.5, 0.0025 * scale)}" text-anchor="middle">${escapeXml(formatDimensionValue(entity, length))}</text>`);
                }
            } else if (entity.type === 'dxf-import') {
                (entity.children || []).forEach(addSvgEntity);
            }
        };
        entities.forEach(addSvgEntity);
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="${left} ${y(top)} ${right - left} ${top - bottom}" width="${widthMm}mm" height="${heightMm}mm"><g>${svgEntities.join('')}</g></svg>`;
        const printWindow = window.open('', '_blank');
        if (!printWindow) {
            showToast('Allow pop-ups to print the PDF.', 'warning', 2200);
            return;
        }
        printWindow.document.open();
        printWindow.document.write(`<!doctype html><html><head><title>${drawingFileName.value || 'cad'} - PDF</title>
            <style>@page{size:${widthMm}mm ${heightMm}mm;margin:0}
            html,body{margin:0;padding:0;background:#121212}
            svg{display:block;width:${widthMm}mm;height:${heightMm}mm}</style>
        ...
        showToast(`PDF print area: ${widthMm.toFixed(1)} × ${heightMm.toFixed(1)} mm at 1:${scale}.`, 'success', 2600);
    }
    ...
    document.getElementById('btn-print-pdf').addEventListener('click', () => {
        imageCaptureSelection = null;
        isPdfPrintMode = true;
        pdfPrintPreviousStatus = statusMode.innerText;
        statusMode.innerText = 'PDF PRINT: DRAG AREA';
        canvas.style.cursor = 'crosshair';
        showToast('Drag over the area to print as PDF. Print scale is applied. Press Escape to cancel.', 'info', 3000);
        render();
    });
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'F3') {
            e.preventDefault();
            toggleDraftingSetting(osnapToggle, 'OSNAP');
        } else if (e.key === 'F8') {
            e.preventDefault();
            toggleDraftingSetting(orthoToggle, 'ORTHO');
        } else if (e.key === 'Enter') {
            if (currentTool === 'pline' && isDrawing) {
                finishPline(false);
            }
            ...
            if (isImageCaptureMode || isPdfPrintMode) {
                ...
                statusMode.innerText = wasPdfPrint ? pdfPrintPreviousStatus : imageCapturePreviousStatus;
                render();
                showToast('Print/capture cancelled.', 'info', 1200);
                return;
            }
            if (hatchCommand) {
                hatchCommand = null;
    ...
    const pointImportNoContours = document.getElementById('point-import-no-contours');
    ...
    const applyPointImportButton = document.getElementById('btn-apply-point-import');
    const updatePointImportMode = () => {
        const importingPoints = document.querySelector('input[name="contour-point-source"]:checked').value === 'new';
        const skipContours = importingPoints && pointImportNoContours.checked;
        pointImportFields.hidden = !importingPoints;
        contourInterval.disabled = skipContours;
        applyPointImportButton.innerText = skipContours ? 'Import Points' : 'Generate Contours';
    };
    ...
    document.getElementById('btn-generate-contours').addEventListener('click', () => {
        ...
        updatePointImportMode();
        if (pointImportFields.hidden) contourInterval.focus();
        else pointImportInput.focus();
    });
    ...
    pointImportNoContours.addEventListener('change', updatePointImportMode);
    ...
    applyPointImportButton.addEventListener('click', () => {
        const importingPoints = document.querySelector('input[name="contour-point-source"]:checked').value === 'new';
        const skipContours = importingPoints && pointImportNoContours.checked;
        const interval = parseStrictFloat(contourInterval.value, NaN);
        if (!skipContours && (!Number.isFinite(interval) || interval <= 0)) {
            showToast('Enter a contour interval greater than zero.', 'warning', 2200);
    ...
    function applyLoadedDrawing(res, entitiesOnly = false, fitInitialView = false) {
        if (res.status === 'success' && res.data) {
            ...
            if (Array.isArray(res.data)) {
                ...
                layers = normalizeLayers(res.data.layers);
                entities.forEach(entity => {
                    if (!layers.includes(entity.layer)) entity.layer = '0';
                });
    ...
    function checkForRemoteChanges() {
        ...
        fetch(apiEndpoint, { method: 'POST', body: formData })
            .then(res => res.json())
            .then(res => {
                if (res.status !== 'success' || !res.revision || res.revision === lastKnownRevision) return;
    ...
    statusZoom.innerText = `ZOOM: ${(camera.zoom * 100).toFixed(0)}%`;
    osnapToggle.addEventListener('change', () => {
        localStorage.setItem('cad_osnap_enabled', String(osnapToggle.checked));
        updateDraftingStatus(osnapToggle, statusOsnap, 'OSNAP');
        updatePropertiesPalette();
    });
    snapGridToggle.addEventListener('change', () => {
        localStorage.setItem('cad_snap_grid_enabled', String(snapGridToggle.checked));
        updateDraftingStatus(snapGridToggle, statusSnapGrid, 'SNAP');
    });
    orthoToggle.addEventListener('change', () => {
        localStorage.setItem('cad_ortho_enabled', String(orthoToggle.checked));
        updateDraftingStatus(orthoToggle, statusOrtho, 'ORTHO');
    });
    statusOsnap.addEventListener('click', () => toggleDraftingSetting(osnapToggle, 'OSNAP'));
    statusSnapGrid.addEventListener('click', () => toggleDraftingSetting(snapGridToggle, 'SNAP'));
    statusOrtho.addEventListener('click', () => toggleDraftingSetting(orthoToggle, 'ORTHO'));
    updateDraftingStatus(osnapToggle, statusOsnap, 'OSNAP');
    updateDraftingStatus(snapGridToggle, statusSnapGrid, 'SNAP');
    updateDraftingStatus(orthoToggle, statusOrtho, 'ORTHO');
    resize();
