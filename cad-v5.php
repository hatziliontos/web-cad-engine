...
(() => {
    ...
    function drawGrid() {
        ...
        // with the drawing around the world origin (0,0), so they stay
        // visually aligned with the rotated entities. We rotate the whole
        // canvas context around the screen projection of world (0,0) instead
        // of re-deriving unbounded rotated grid lines, then draw the grid
        // exactly as before in this rotated frame.
        ...
        // Compute the world-space view bounds generously so the rotated
        // grid still covers the full visible canvas (corners can map to
        // world points further away once rotated).
        const corners = [
            screenToWorld(0, 0),
        ...
        corners.forEach(c => { cxWorld += c.x; cyWorld += c.y; });
        cxWorld /= corners.length;
        cyWorld /= corners.length;

        const startX = Math.floor((cxWorld - diag) / gridSize) * gridSize;
        const endX = Math.ceil((cxWorld + diag) / gridSize) * gridSize;
        const startY = Math.floor((cyWorld - diag) / gridSize) * gridSize;
        const endY = Math.ceil((cyWorld + diag) / gridSize) * gridSize;
        ...
        ctx.beginPath(); ctx.moveTo(orig.x - diag * camera.zoom, orig.y); ctx.lineTo(orig.x + diag * camera.zoom, orig.y); ctx.stroke();
        ctx.strokeStyle = 'rgba(34, 197, 94, 0.4)';
        ctx.beginPath(); ctx.moveTo(orig.x, orig.y - diag * camera.zoom); ctx.lineTo(orig.x, orig.y + diag * camera.zoom); ctx.stroke();
