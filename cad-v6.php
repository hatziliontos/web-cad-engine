...
(() => {
    ...
    function getDrawingPayload() {
        return {
            entities: entities,
            canvasRotationAngleRad: canvasRotationAngleRad
        };
    }
    ...
    function rotateAllEntities(deltaAngleRad) {
        // Positive angle = clockwise rotation (matches DXF $ANGDIR=1 convention),
        // so negate the angle for the standard CCW rotation matrix below.
        const cosA = Math.cos(-deltaAngleRad);
        const sinA = Math.sin(-deltaAngleRad);
        
        // Rotation pivot is always the world origin (0,0), so that world
        // coordinates of the drawing, the grid and the North indicator stay
        // consistent with each other after rotation. The paper frame is
        // intentionally NOT rotated (handled separately, unaffected here).
        const centerX = 0;
        const centerY = 0;
        ...
        // Rotate all entities around center
        entities.forEach(ent => {
            if (ent.type === 'point') {
                const rotated = rotatePoint(ent.x, ent.y);
                ent.x = rotated.x;
                ent.y = rotated.y;
            } else if (ent.type === 'line') {
                const p1 = rotatePoint(ent.x1, ent.y1);
                const p2 = rotatePoint(ent.x2, ent.y2);
                ent.x1 = p1.x; ent.y1 = p1.y;
                ent.x2 = p2.x; ent.y2 = p2.y;
            } else if (ent.type === 'rect') {
                const p1 = rotatePoint(ent.x, ent.y);
                const p2 = rotatePoint(ent.x + ent.w, ent.y + ent.h);
                ent.x = Math.min(p1.x, p2.x);
                ent.y = Math.min(p1.y, p2.y);
                ent.w = Math.abs(p2.x - p1.x);
                ent.h = Math.abs(p2.y - p1.y);
            } else if (ent.type === 'circle') {
                const rotated = rotatePoint(ent.cx, ent.cy);
                ent.cx = rotated.x;
                ent.cy = rotated.y;
                ...
                ent.x1 = p1.x; ent.y1 = p1.y;
                ent.x2 = p2.x; ent.y2 = p2.y;
                if (ent.textX !== undefined && ent.textY !== undefined) {
                    const textRotated = rotatePoint(ent.textX, ent.textY);
                    ent.textX = textRotated.x;
                    ent.textY = textRotated.y;
                }
            }
        });
        
        canvasRotationAngleRad = normalizeAngle(canvasRotationAngleRad + deltaAngleRad);
    }
    ...
    function drawGrid() {
        // The grid (and the red/green X=0/Y=0 axis lines) rotate together
        // with the drawing around the world origin (0,0). We rotate grid
        // points in world-space math (same convention as rotateAllEntities)
        // rather than using a canvas ctx.rotate() transform: a canvas
        // transform pivots around the *screen pixel* of world (0,0), which
        // can be extremely far away when the camera is panned far from the
        // origin (typical for real survey coordinates), causing the whole
        // grid to be flung off-screen for even a small rotation angle.
        const angle = canvasRotationAngleRad;
        const cosA = Math.cos(-angle);
        const sinA = Math.sin(-angle);
        const cosInv = Math.cos(angle);
        const sinInv = Math.sin(angle);
        // Forward: unrotated (u,v) -> rotated world (x,y)
        const toWorld = (u, v) => ({ x: u * cosA - v * sinA, y: u * sinA + v * cosA });
        // Inverse: rotated world (x,y) -> unrotated (u,v)
        const toLocal = (x, y) => ({ x: x * cosInv - y * sinInv, y: x * sinInv + y * cosInv });

        const gridSize = getGridSize();

        // Find the unrotated-space bounds that cover the current view by
        // inverse-rotating the screen corners' world coordinates.
        const corners = [
            screenToWorld(0, 0),
            screenToWorld(canvas.width, 0),
            screenToWorld(0, canvas.height),
            screenToWorld(canvas.width, canvas.height)
        ].map(c => toLocal(c.x, c.y));
        const diag = Math.hypot(canvas.width, canvas.height) / camera.zoom;

        let minU = Infinity, maxU = -Infinity, minV = Infinity, maxV = -Infinity;
        corners.forEach(c => {
            minU = Math.min(minU, c.x); maxU = Math.max(maxU, c.x);
            minV = Math.min(minV, c.y); maxV = Math.max(maxV, c.y);
        });
        minU -= gridSize; maxU += gridSize;
        minV -= gridSize; maxV += gridSize;

        const startU = Math.floor(minU / gridSize) * gridSize;
        const endU = Math.ceil(maxU / gridSize) * gridSize;
        const startV = Math.floor(minV / gridSize) * gridSize;
        const endV = Math.ceil(maxV / gridSize) * gridSize;
        ...
        ctx.strokeStyle = '#222';
        ctx.beginPath();
        for (let u = startU; u <= endU; u += gridSize) {
            const p1 = screenOf(u, startV);
            const p2 = screenOf(u, endV);
            ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y);
        }
        for (let v = startV; v <= endV; v += gridSize) {
            const p1 = screenOf(startU, v);
            const p2 = screenOf(endU, v);
            ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y);
        }
        ctx.stroke();
        ...
        if (gridSpacingPx >= 30) {
            ...
            for (let u = startU; u <= endU; u += gridSize) {
                const sp = screenOf(u, 0);
                if (sp.x >= 2 && sp.x <= canvas.width - 2 && sp.y >= 2 && sp.y <= canvas.height - 2) {
                    ctx.textAlign = 'center';
                    ctx.fillText(u.toFixed(gridLabelDecimals), sp.x, canvas.height - 14);
                }
            }
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            for (let v = startV; v <= endV; v += gridSize) {
                const sp = screenOf(0, v);
                if (sp.y >= 2 && sp.y <= canvas.height - 2 && sp.x >= 2 && sp.x <= canvas.width - 2) {
                    ctx.fillText(v.toFixed(gridLabelDecimals), 4, sp.y);
                }
            }
            ctx.restore();
        }
        ...
        ctx.beginPath();
        {
            const p1 = screenOf(startU, 0), p2 = screenOf(endU, 0);
            ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y);
        }
        ctx.stroke();
        ...
        ctx.beginPath();
        {
            const p1 = screenOf(0, startV), p2 = screenOf(0, endV);
            ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y);
        }
        ctx.stroke();
    }
    ...
    function drawNorthSymbol() {
        ...
        // The arrow (and its "Β" label) point to true North, so they rotate
        // together with the drawing/grid by the canvas rotation angle.
        // Canvas rotate() is clockwise for positive angles, matching the
        // clockwise-positive convention used by rotateAllEntities().
        ctx.save();
        ctx.translate(cx, cy);
        ctx.rotate(canvasRotationAngleRad);
        ctx.translate(-cx, -cy);
    ...
    function bindCanvasRotationInput() {
        const rotationInput = document.getElementById('prop-canvas-rotation');
        if (!rotationInput) return;
        rotationInput.addEventListener('keypress', (event) => {
            if (event.key === 'Enter') {
                const value = parseStrictFloat(rotationInput.value, NaN);
                if (!Number.isFinite(value)) {
                    rotationInput.value = azimuthRadToValue(canvasRotationAngleRad).toFixed(4);
                    showToast('Please enter a valid angle.', 'error', 1500);
                    return;
                }
                const newAngleRad = azimuthValueToRad(value);
                const deltaAngleRad = newAngleRad - canvasRotationAngleRad;
                if (Math.abs(deltaAngleRad) < 1e-9) {
                    showToast('Rotation angle unchanged.', 'info', 1500);
                    return;
                }
                saveState();
                rotateAllEntities(deltaAngleRad);
                render();
                updatePropertiesPalette();
                showToast(`Canvas rotated to ${value}${getAngleUnitLabel()}.`, 'success', 1500);
            }
        });
    }
    ...
    function updatePropertiesPalette() {
        let html = `
            <div class="prop-group">
                ...
                <div class="prop-row">
                    <label for="prop-canvas-rotation">Canvas Rotation (${getAngleUnitLabel()})</label>
                    <input type="text" id="prop-canvas-rotation" value="${azimuthRadToValue(canvasRotationAngleRad).toFixed(4)}">
                </div>
            </div>
        `;
    ...
    function applyLoadedDrawing(res, entitiesOnly = false, fitInitialView = false) {
        if (res.status === 'success' && res.data) {
            ...
            if (Array.isArray(res.data)) {
                entities = res.data;
                canvasRotationAngleRad = 0;
            } else {
                entities = res.data.entities || [];
                canvasRotationAngleRad = Number(res.data.canvasRotationAngleRad) || 0;
